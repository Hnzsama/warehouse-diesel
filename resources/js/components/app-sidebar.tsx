    ...
    return (
        <Sidebar collapsible="icon" variant="inset">
                            ...
                            <Link href={dashboard()} prefetch className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col gap-1 leading-none">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        <span className={`inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-bold tracking-wide uppercase border ${
                                            isAdmin
                                                ? 'bg-red-500/10 text-red-600 border-red-500/20 dark:text-red-400'
                                                : 'bg-amber-500/10 text-amber-600 border-amber-500/20 dark:text-amber-400'
                                        }`}>
                                            <ShieldCheck className="h-3 w-3 shrink-0" />
                                            <span>{isAdmin ? 'Admin Gudang' : 'Pemilik (Owner)'}</span>
                                        </span>
                                    </div>
                                </div>
                            </Link>
            ...
            <SidebarContent className="px-2 py-1">
                ...
                {/* Master Data (Khusus Admin Gudang) */}
                {isAdmin && (
                        ...
                        <SidebarMenu>
                            ...
                            <SidebarMenuItem>
                                <SidebarMenuButton
                                    ...
                                    tooltip="Kelola Admin Gudang"
                                    className="rounded-lg font-medium text-sm gap-3 px-3 py-2.5 transition-colors"
                                >
                                    <Link href="/users">
                                        <UserCheck className="h-4 w-4" />
                                        <span>Kelola Admin Gudang</span>
                                    </Link>
                                </SidebarMenuButton>
                            </SidebarMenuItem>
                        </SidebarMenu>
                ...
                {/* Kelola Pengguna (Jika role Owner) */}
                {!isAdmin && isOwner && (
                    <SidebarGroup className="mt-2">
                        <SidebarGroupLabel className="text-[11px] font-bold tracking-wider uppercase text-muted-foreground/70 px-3 mb-1">
                            Manajemen Akses
                ...
                {/* Transaksi Stok (Khusus Admin Gudang) */}
                {isAdmin && (
                    <SidebarGroup className="mt-2">
                        <SidebarGroupLabel className="text-[11px] font-bold tracking-wider uppercase text-muted-foreground/70 px-3 mb-1">
                            Transaksi Stok (Admin)
                        </SidebarGroupLabel>
                        <SidebarMenu>
                            <SidebarMenuItem>
                                <SidebarMenuButton
                ...
                {/* Laporan & Monitoring (Dua Role: Admin & Pemilik) */}
                <SidebarGroup className="mt-2">
                    <SidebarGroupLabel className="text-[11px] font-bold tracking-wider uppercase text-muted-foreground/70 px-3 mb-1">
                        Laporan & Monitoring
                    </SidebarGroupLabel>
